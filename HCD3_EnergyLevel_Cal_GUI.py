...
from HCD3_EnergyLevel_Cal_Core import (
    HotWarmDispenserInput, ColdHotDispenserInput,
    evaluate, evaluate_cold_hot
)
...
LANGUAGES = {
    ...
    'en': {
        ...
        'type_label': 'Dispenser Type:',
        'type_hot_warm': 'Hot-Warm Type',
        'type_cold_hot': 'Cold-Hot Type',
        ...
        'energy_label_label': 'Energy Label:',
...
class CNS3910CalculatorV2:
    def __init__(self, root):
        self.root = root
        self.current_lang = 'zh'
        self.dispenser_type = 'cold_hot'  # 默认冰温热型
    ...
    def create_widgets(self):
        ...
        self.create_type_selector(main_container)
    ...
    def create_type_selector(self, parent):
        type_frame = tk.Frame(parent, bg=self.colors['frame_bg'], relief='solid', borderwidth=1)
        type_frame.pack(fill='x', pady=(0, 15), padx=2, ipady=8)
        
        lang = LANGUAGES[self.current_lang]
        
        self.type_label = tk.Label(
            type_frame,
            text=lang['type_label'],
            font=('Arial', 11, 'bold'),
            bg=self.colors['frame_bg']
        )
        self.type_label.pack(side='left', padx=20)
        
        self.type_var = tk.StringVar(value='cold_hot')
        
        self.radio_hot_warm = tk.Radiobutton(
            type_frame,
            text=lang['type_hot_warm'],
        ...
        self.radio_hot_warm.pack(side='left', padx=10)
        
        self.radio_cold_hot = tk.Radiobutton(
            type_frame,
            text=lang['type_cold_hot'],
            variable=self.type_var,
            value='cold_hot',
            font=('Arial', 11),
            bg=self.colors['frame_bg'],
        ...
        self.radio_cold_hot.pack(side='left', padx=10)
    ...
    def update_input_fields(self):
        ...
        if self.type_var.get() == 'hot_warm':
            # 温热型输入字段
            fields = [
                ('E24', 'E24_label', '1.152'),
                ('T_hot', 'T_hot_label', '87.0'),
                ('T_amb', 'T_amb_label', '25.0'),
                ('V_hot', 'V_hot_label', '1.4'),
            ]
        else:
            # 冰温热型输入字段
            fields = [
        ...
        for key, label_key, default in fields:
            self.add_input_field(key, lang[label_key], default)
    ...
    def on_type_change(self):
        self.dispenser_type = self.type_var.get()
        self.update_input_fields()
        # 清空结果
    ...
    def calculate_hot_warm(self):
        ...
        self.display_hot_warm_results(result)
    
    def calculate_cold_hot(self):
        lang = LANGUAGES[self.current_lang]
        ...
        self.display_cold_hot_results(result)
    
    def display_hot_warm_results(self, result):
        for widget in self.result_frame.winfo_children():
            widget.destroy()
    ...
    def display_cold_hot_results(self, result):
        for widget in self.result_frame.winfo_children():
            widget.destroy()
    ...
    def display_grade_row(self, grade, result):
        ...
        # 根据结果类型选择正确的E24字段
        if 'E_st24_kWh' in result:
            # 温热型
            E24_value = result['E_st24_kWh']
        else:
            # 冰温热型
    ...
    def display_energy_label_result(self, result):
        lang = LANGUAGES[self.current_lang]
        ...
        label_label = tk.Label(
            final_frame,
            text=lang['energy_label_label'],
            font=('Arial', 12, 'bold'),
            bg='#f9fafb'
        )
        label_label.pack(pady=(12, 5))
        ...
        if result['is_qualified']:
            ...
            margin_label = tk.Label(
                final_frame,
                text=f"{lang['margin']} {result['margin_kWh']:.3f} {lang['kWh_unit']} ({result['margin_percent']:.1f}%)",
                font=('Arial', 11),
                bg='#f9fafb',
                fg=self.colors['success']
            )
            margin_label.pack(pady=(0, 12))
        else:
            fail_label = tk.Label(
                final_frame,
                text=lang['not_qualified'],
            ...
            exceed = abs(result['margin_kWh'])
            exceed_label = tk.Label(
                final_frame,
                text=f"+{exceed:.3f} {lang['kWh_unit']} ({abs(result['margin_percent']):.1f}%)",
                font=('Arial', 11),
                bg='#f9fafb',
                fg=self.colors['danger']
            )
